...
void updateSound() {
  static unsigned long totalSound = 0;
  ...
  sound = (int) (readADC(SOUND_PIN) * 0.097 );
  ...
  //Serial.print( (int) (readADC(SOUND_PIN) * 0.097 ) );

  //totalSound += (int) (readADC(SOUND_PIN) * 0.097 );

  // Compare time
  if (sampleCount = AUDIO_SAMPLE_AMOUNT) {
    //lastSoundReadTime = currentMillis;

    //totalSound -= 600;
    
    // Smoothed LED brightness
    int brightness = map(totalSound, 0, 4 * AUDIO_SAMPLE_AMOUNT, 0, 255);
    CRGB color = ColorFromPalette(soundPalette, brightness);
